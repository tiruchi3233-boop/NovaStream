...
// 1. ट्रेंडिंग फिल्में फैच (Fetch) करने का मुख्य फंक्शन
...
function displayMovies(movies) {
    // index.html में मौजूद कैरोसैल या कंटेनर को ढूँढना
    const container = document.getElementById('movies-container') || document.querySelector('.carousel');
    ...
    container.innerHTML = movies.map(movie => `
        <div class="movie-card" onclick="playTrailer(${movie.id})">
            <img src="${movie.poster_path ? IMAGE_URL + movie.poster_path : 'https://via.placeholder.com/180x250'}" alt="${movie.title}">
            <h3>${movie.title}</h3>
        </div>
    `).join('');
}
...
// 4. पेज लोड होते ही ऑटोमैटिकली डेटा फैच करें
